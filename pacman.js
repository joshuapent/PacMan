...
const pointZoneSetup = document.getElementById('pointZone');
const pointZone = pointZoneSetup.getContext('2d');
...
class Sprite {
    ...
    pacmanConnect() { 
        ...
        this.points();
        this.powerOrb();
        if (pointsCollected === 325) return console.log("You win!")
    ...
    points() {
        for (let i = 0; i < this.pointLocation.length; i++) {
            if (this.pointLocation[i][0] < pacman.xAxis + 27 && this.pointLocation[i][0] > pacman.xAxis +5 && this.pointLocation[i][1] < pacman.yAxis + 27 && this.pointLocation[i][1] > pacman.yAxis +5) {
                if (this.pointLocation[i] === [0, 0]) return;
                else {
                    pointZone.clearRect(this.pointLocation[i][0], this.pointLocation[i][1], 4, 4)
...
sprites.onload = function() { //sets up all of the assets on website load, some are on timers.
    background.drawImage(backgroundImg, 0, 0)
    pointZone.drawImage(pointZoneImg, 0, 0)
    setTimeout(() => {
        game.drawImage(sprites, 0, 0, 32, 32, pacman.xAxis, pacman.yAxis, 32, 32)
        ghostZone.drawImage(sprites, 0, 32, 32, 32, redGhost.xAxis, redGhost.yAxis, 32, 32)
        ghostZone.drawImage(sprites, 0, 64, 32, 32, pinkGhost.xAxis, pinkGhost.yAxis, 32, 32)
...
pointZoneImg.src = 'images/points.png';
